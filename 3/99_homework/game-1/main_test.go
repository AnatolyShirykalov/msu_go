	...
	for caseNum, commands := range game0cases {
		...
		go func() {
			...
			for lastOutput["Tristan"] = range output {
				fmt.Println("3330", lastOutput["Tristan"])
			}
			fmt.Println("777")
		}()
		initGame()
...
// поскольку предполагается, что комнады отправляются асинхронно, то в ответах тестовых кейсов - последний ответ, полученный данным игроком
